...
void PictureModel::set_all_checked_for_on_editing(bool checked)
{
...
void PictureModel::set_one_checked_for_on_editing(const QString &picture_id,bool checked)
{
    QString checked_key = ImagesTable::getInstance()->get_field_name(ImagesTable::Field::Checked_For_Add_To_Paint_List_Bool);
    int role = roles.key(checked_key.toLatin1());
...
void PictureModel::set_one_checked_for_on_editing(int index_,bool checked)
{
    QString checked_key = ImagesTable::getInstance()->get_field_name(ImagesTable::Field::Checked_For_Add_To_Paint_List_Bool);
    int role = roles.key(checked_key.toLatin1());
...
//void PictureModel::clear_full_list()
//{
////    clear();
...
void PictureModel::checked_export_or_delete(bool checked)
{
    for(int i = 0; i < _selected_data.size();i++)
    {
...
//void PictureModel::append_to_full_list( QJsonObject *info)
//{
...
void PictureModel::append(const QJsonObject &info)
{
    QString picture_key = ImagesTable::getInstance()->get_field_name(ImagesTable::Field::Image_ID_TEXT);
    const QString & new_picture_id = info[picture_key].toString();
    bool found = false;
    for(int i =0 ;i < _selected_data.size();i ++)
    {
        const QJsonObject & ele = _selected_data.at(i);
        const QString & has_picture_id = ele[picture_key].toString();
        if(has_picture_id == new_picture_id)
        {
            found    = true;
            break;
        }
    ...
    if(found == true)
    {
        Log::i(TAG,"图片id已经存在，不再添加进去:"+new_picture_id);
        return;
    }
...
void PictureModel::remove_first()
{
...
void PictureModel::remove(const QString &picture_id)
{
...
//void PictureModel::remove(QJsonObject *info)
//{
//    for(int i =0 ;i < _selected_data.size();i ++)
//    {
//        const QJsonObject & ele = _selected_data.at(i);
...
void PictureModel::delete_checked_pictures(std::map<QString,QString> & out)
{
...
const QList<QJsonObject> &PictureModel::get_data() const
{
...
bool PictureModel::update_property(const QString & picture_id, const QJsonObject & new_propertys)
{
    QString picture_key = ImagesTable::getInstance()->get_field_name(ImagesTable::Field::Image_ID_TEXT);
...
void PictureModel::checked_thoes_ids(const QStringList &picture_id)
{
   QString picture_key = ImagesTable::getInstance()->get_field_name(ImagesTable::Field::Image_ID_TEXT);
...
//void PictureModel::select(ImagesTable::PictureSoureType picture_soure_type, ImagesTable::PictureLevel picture_level, ImagesTable::PictureHasTeeType picture_has_tee_type, qint64 begin, qint64 end, const QJsonArray &tee_indexs,const QString &tee_type, const QList<QJsonObject> & all_data)
//{
//    clear();
//    for(auto  v : all_data )
//    {
...
//bool PictureModel::test_filter(const QJsonObject &picture, ImagesTable::PictureSoureType picture_soure_type, ImagesTable::PictureLevel picture_level, ImagesTable::PictureHasTeeType picture_has_tee_type, qint64 begin, qint64 end, const QJsonArray &tee_indexs)
//{
...
int PictureModel::getCount() const
{
    return _count;
}

void PictureModel::setCount(int count)
{
    if(_count != count)
    {
        _count = count;
...
void PictureModel::get_checked_files(QStringList &out) const
{
...
//QJsonObject * PictureModel::update_property(const QString & picture_id,const QJsonObject & new_propertys)
//{
//    QString picture_key = ImagesTable::getInstance()->get_field_name(ImagesTable::Field::Image_ID_TEXT);
...
//PictureModel::TodayCollectedPictureModel::TodayCollectedPictureModel(QObject *parent):QAbstractListModel(parent)
//{
...
//int PictureModel::TodayCollectedPictureModel::rowCount(const QModelIndex &parent) const
//{
//    Q_UNUSED(parent);
//    return _selected_data.count();
//}

//QVariant PictureModel::TodayCollectedPictureModel::_data(const QModelIndex &index, int role) const
//{
//    int row = index.row();
//    if(row < 0 || row >= _selected_data.count()) {
//        return QVariant();
...
//void PictureModel::TodayCollectedPictureModel::clear()
//{
//    beginResetModel();
//    _selected_data.clear();
//    endResetModel();
...
//void PictureModel::TodayCollectedPictureModel::append(QJsonObject *info)
//{
//    int size = _selected_data.size();
//    beginInsertRows(QModelIndex(), size, size);
//    _selected_data.append(info);
...
//QHash<int, QByteArray> PictureModel::TodayCollectedPictureModel::roleNames() const
//{
//    return roles;
//}
